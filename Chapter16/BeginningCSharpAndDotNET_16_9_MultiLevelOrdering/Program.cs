...
var queryResult = from c in customers orderby c.Region, c.Country, c.City select new { c.ID, c.Region, c.Country, c.City };

foreach (var result in queryResult)
{

    Console.WriteLine($"{result.ID} {result.Region} {result.Country} {result.City}");
}


var querryResult = from c in customers group c by c.Region into cg select new { TotalSales = cg.Sum(c => c.Sales), Region = cg.Key };

var orededResults = from cg in querryResult orderby cg.TotalSales descending select cg;

Console.WriteLine("Total\t: By\nSales\t: Region\n-----\t----");

foreach (var result in orededResults)
{

    Console.WriteLine($"{result.TotalSales}\t: {result.Region}");
}